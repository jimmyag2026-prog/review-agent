...
Does all the work deterministically:
  ...
  6. Prints the reply text to stdout — Orchestrator sends via send-lark.sh
...
def find_session_dir(session_id: str) -> Optional[Path]:
    root = Path(os.environ.get("REVIEW_AGENT_ROOT", Path.home() / ".review-agent"))
    for p in (root / "users").glob(f"*/sessions/{session_id}"):
        if p.is_dir():
            return p
    return None
...
def main():
    ...
    #   stdout = IM reply text ONLY (main agent pipes this to send-lark.sh)
