...
def list_users():
    ...
    for u in sorted(udir.iterdir()):
        ...
        try:
            ...
            sessions = list_sessions(u.name)
            active = sum(1 for s in sessions if s["status"] != "closed")
            closed = sum(1 for s in sessions if s["status"] == "closed")
            out.append({
                "open_id": u.name,
                "display_name": meta.get("display_name") or u.name,
                "roles": meta.get("roles", []),
                "responder": meta.get("responder"),
                "created_at": meta.get("created_at"),
                "active_count": active,
                "closed_count": closed,
                "has_profile": (u / "profile.md").exists(),
                "has_active_pointer": (u / "active_session.json").exists(),
            })
        except Exception: pass
    return out


def list_sessions(open_id):
    udir = ROOT / "users" / open_id
    sdir = udir / "sessions"
    out = []
...
def find_session(session_id):
    for u in (ROOT / "users").glob("*/sessions/" + session_id):
        if u.is_dir():
            return u
    return None
