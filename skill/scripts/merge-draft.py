...
如果要我再改某处，告诉我具体哪里。"""
        send = SKILL_DIR / "scripts" / "send-lark.sh"
        import subprocess
        r = subprocess.run([str(send), "--open-id", req_oid, "--text", preview],
        ...
        if r.returncode == 0:
            ...
            print("  preview sent to Requester")
        else:
            print(f"  preview send failed: {r.stderr}", file=sys.stderr)
